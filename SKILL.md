...
> v3 关键升级:**引入 4+1 层信息分层模型(基于于一框架)、新增个人经验层 L1 目录、新增高频场景路由表、frontmatter 强制 layer 字段**。
>
...
> 基于于一公开分享的 4 层分类法 + 本 Skill 第 5 层冷藏的扩展。完整思想见 [PHILOSOPHY.md](./PHILOSOPHY.md#二4-1-层信息分层模型本-skill-的核心)。
...
- **v3.0**(2026-05): 引入 4+1 层信息分层模型(于一框架 + 冷藏扩展)、新增 L1 个人经验层及三个子目录、新增高频场景路由表(12 场景)、frontmatter 强制 layer 字段、新增独立 PHILOSOPHY.md 思想文档
